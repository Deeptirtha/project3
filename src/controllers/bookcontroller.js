...
const createBooks = async function (req, res) {
  try {
    ...
    if (!validator.validTitleBooks.test(title.trim())) return res.status(400).send({ status: false, message: "provide a valid Book Title " })
    ...
    if (!validator.validName.test(excerpt.trim())) return res.status(400).send({ status: false, message: "provide excerpt is string" })

    if (!userId) return res.status(400).send({ status: false, message: "Please Provide userId" })
    data.userId = data.userId.trim()

    if (!ISBN) return res.status(400).send({ status: false, message: "Please Provide ISBN" })
    if (!validator.ValidISBN.test(ISBN.trim())) return res.status(400).send({ status: false, message: "Please Provide Valid ISBN like this [10 or 13 digits] and hyphens" })
    ...
    if (!validator.validcategory.test(category)) return res.status(400).send({ status: false, message: "please provide category string" })

    if (!subcategory) return res.status(400).send({ status: false, message: "Please Provide Subcategory" })
    if (!validator.validName.test(subcategory)) return res.status(400).send({ status: false, message: "please provide subcategory string" })

    if (!releasedAt) return res.status(400).send({ status: false, message: "Please Provide releasedAt" })
    if (!validator.validTime.test(releasedAt.trim())) return res.status(400).send({ status: false, message: "Please enter releasedAt in the right format(YYYY-MM-DD)!" })
