...
const createBooks = async function (req, res) {
    try {
        ...
        if (!validator.validName(title)) return res.status(400).send({ status: false, message: "provide title in string" })
...
const getBookData = async function (req, res) {
    try {
      ...
      if (!Id) {
        let result = await bookModel.find(data).select({ _id: 1, title: 1, excerpt: 1, userId: 1, category: 1, reviews: 1, releasedAt: 1, }).sort({ title: 1 })
