...
let creatUser= async function (req,res){
   ...
   if(!data.title.trim() || validTitle(data.title)){return res.status(400).send({status:false,message:"enter a valid title"})}
   if(!data.email.trim() || !validEmail.test(data.email)){return res.status(400).send({status:false,message:"enter a valid email"})}
   if(!data.phone.trim() || !validMobile.test(data.phone)){return res.status(400).send({status:false,message:"enter a valid phone No"})}
   ...
   if(!address.street.trim()){return res.status(400).send({status:false,message:"street is mandatory in your address "})}
   if(!address.city.trim()){return res.status(400).send({status:false,message:"city is mandatory in your address "})}
   if(!address.pincode.trim() || !validpincode.test(address.pincode)||(address.pincode.length!=6)){return res.status(400).send({status:false,message:"please input valid pincode "})}
...
const loginUser=async function(req,res){
 try{   let data=req.body
    ...
    let token =jwt.sign({ID:user._id},"project3-group9",{ expiresIn:"1h"})
